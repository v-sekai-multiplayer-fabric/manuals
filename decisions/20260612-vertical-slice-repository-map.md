...
- [`godot-images`](https://github.com/v-sekai-multiplayer-fabric/godot-images) — the engine build factory and its container toolchain. A GitHub Actions matrix cross-compiles the double-precision fork's editor and export templates (`editor`, `template_release`, `template_debug`) for Windows (through the llvm-mingw toolchain) and Linux, with sccache backed by GitHub Actions cache, and publishes each platform as a release zip (`windows-editor.zip`, `windows-template-release.zip`, …) that `godot-loop-slice` unpacks for its MSIX export. A parallel `build-docker` job builds the rootless podman quadlet Linux server images to GHCR. The native Linux engine/template builds are currently paused ([disabled in #28](https://github.com/v-sekai-multiplayer-fabric/godot-images/pull/28)), so releases ship Windows-only until they are re-enabled.
